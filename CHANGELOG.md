...
**Validation.** Per design §19.1, the authority order is Ordo Recitandi → governing rubrical books (1911 / 1955 / 1960) → legacy Divinum Officium Perl output. `composeHour()` runs exception-free across every Hour for every date in 2024 under each of the three Roman policies (8,784 compositions per run) via the `test:no-throw` integration test. The live Perl comparison harness at `pnpm -C packages/compositor compare:phase-3-perl` now surfaces a `Matching prefix` metric per row and per-policy best/average summaries — forward progress is visible even when row counts stay flat. Current ledger state is: Divino Afflatu `496` divergent hours with `456` still unadjudicated (best/average matching prefix `4/2.9` lines), Reduced 1955 `488` with `451` unadjudicated (`126/30.8`), and Rubrics 1960 `488` with `451` unadjudicated (`126/31.3`). Classifications live in [`packages/compositor/test/divergence/adjudications.json`](packages/compositor/test/divergence/adjudications.json) per [ADR-011](docs/adr/011-phase-3-divergence-adjudication.md), with chronological pattern resolutions in [`packages/compositor/test/divergence/ADJUDICATION_LOG.md`](packages/compositor/test/divergence/ADJUDICATION_LOG.md) and upstream `perl-bug` families tracked in [`docs/upstream-issues.md`](docs/upstream-issues.md).

Latest 3h burn-down work closed the Reduced 1955 Epiphany / Baptism minor-hour later-block checkpoint and tightened the shared Roman January Matins checkpoint without reopening Vespers, dirge, or Divino Afflatu implementation. On the code side, the Matins planner now selects Roman January antiphons against the visible conditionalized content rather than raw top-level nodes, and Phase 3 reference resolution now carries season/version context so integer selectors land on the same visible nodes during composition. That moves Jan `6/13` Roman Matins onto the proper Epiphany first-nocturn antiphon/psalm source seam while leaving the remaining Jan `6/13/14` Matins divergences cleanly exposed as opener, invitatory-handoff, and later-block families. For the Reduced 1955 Jan `6/7` minor hours, `applyRuleSet()` now restores the real office-backed later blocks by resolving `Lectio Prima` / `Capitulum Prima` and `Responsory Breve {Tertia,Sexta,Nona}` instead of falling through to the empty Ordinarium wrapper. The live compare then advanced those rows to the proper lesson / `R.br.` lines, and the remaining Jan `6/7` differences are now adjudicated as source-backed `perl-bug`s because `Sancti/01-06.txt` and `Tempora/Epi1-0.txt` explicitly carry the later-block sections that Perl omits.
