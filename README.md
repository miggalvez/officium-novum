...
| **4 — API** | In progress — 4b scaffold and metadata endpoints shipped in `@officium-novum/api`. Fastify app/server setup, OpenAPI 3.1 route generation, `/api/v1/status`, `/api/v1/versions`, `/api/v1/languages`, explicit supported/deferred/missa-only version classification, `la`/`en` language mapping, and `orthography=source|version` parsing are in place. The office composition route is next. |
