...
**Phase 3 (Composition Engine) is complete.** All eight sub-phases (3a–3h) ship; `composeHour()` runs end-to-end for every Hour under the three Roman policies. Regressions are gated by the 8,784-composition no-throw sweep, the 312 Appendix-A snapshot goldens at `packages/compositor/test/__goldens__/`, and `pnpm -C packages/compositor verify:phase-3-signoff`. All three Roman ledgers report **0 unadjudicated** rows. Per ADR-011, every adjudicated ledger row carries a class + citation in `packages/compositor/test/divergence/adjudications.json`; no divergence is ever resolved by "matching the Perl" alone.
